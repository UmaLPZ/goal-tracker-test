...
public class GoalManager implements ReorderableList<Goal>
{
    ...
    public List<NpcKillTask> getAllNpcKillTasks() {
        List<NpcKillTask> tasks = new ArrayList<>();
        for (Goal goal : goals) {
            for (Task task : goal.getAll()) {
                if (task instanceof NpcKillTask) {
                    tasks.add((NpcKillTask) task);
                }
            }
        }
        return tasks;
    ...
    public List<NpcKillTask> getAllIncompleteNpcKillTasks() {
        return getAllIncompleteTasksOfType(TaskType.NPC_KILL);
    }

    public NpcKillTask getOrCreateNpcKillTask(String npcName) {
        for (NpcKillTask task : getAllNpcKillTasks()) {
            if (task.getNpcName().equals(npcName)) {
                return task;
            }
        }

        Goal newGoal = createGoal(); // Create new goal
        NpcKillTask task = new NpcKillTask(newGoal);
        task.setNpcName(npcName);
        newGoal.add(task); // Add task to the new goal
        return task;
    }
